...
@shared_task()
def new_stories_from_twitter(account): #twitter timeline scrapper for analytics
...
#check if the stroy is currently being processes
...
#Function to exract image urls (if any) present inside a library post
@shared_task
...
def save_story(url, channel, pub_time, title=None, account=0, feed_id=None, story_update=False):
    ...
    try:
        ...
        #keyword_populate(s)
    except IntegrityError as e: #handling the case when multiple threads process the same article and try to save the same article simulataneously
...
#funtion that copies the image files downloaded from a web page onto the correct directory
...
#function to predict the keywords using data in meta tags
...
#function to fetch the articles from a user's blog feeds
@shared_task
